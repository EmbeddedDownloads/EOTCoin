...
// Copyright (c) 2011-2012 EOTCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EOTCoinTEST0\r");
            Send(hSocket, "WHO #EOTCoinTEST0\r");
        } else {
            // randomly join #EOTCoin00-#EOTCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EOTCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EOTCoin%02d\r", channel_number).c_str());
        }
