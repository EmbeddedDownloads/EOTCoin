...
uint256 hashGenesisBlock("0x4623f2326719da75d6791b33f29b3b3e7c1be2501038c7c21414d5a72f6b5380");
...
const string strMessageMagic = "EOTCOIN Signed Message:\n";
...
{
    // EOTCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // EOTCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "eot genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1499635372;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1477257;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x13f649673b487c5cc7ed1d2229268cbffe1b44f9ed0ec890e15031f3cd62bdad"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EOTCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EOTCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EOTCoinMiner(CWallet *pwallet)
{
    printf("EOTCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EOTCoin-miner");
    ...
    try { loop {
        ...
        printf("Running EOTCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EOTCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EOTCoinMiner, pwallet));
}
